      ...
      <div class="card-header d-flex align-items-center justify-content-between" style="margin-bottom: -0.7rem;">
        <div class="justify-content-start">
          <button type="button" class="btn btn-xs btn-dark fw-bold p-2" data-bs-toggle="modal" data-bs-target="#formModalAdminAddPengguna">
            <i class='bx bxs-user-account fs-6'></i>&nbsp;ADD USER
          </button>
        </div>
        <div class="justify-content-end">
          ...
          <form action="/admin/pengguna/search">
            <div class="input-group">
              <input type="search" class="form-control" name="q" id="search" style="border: 1px solid #d9dee3;" value="{{ request('q') }}" placeholder="Search Users Data..." autocomplete="off" />
            </div>
          </form>
          <!-- /Search -->
        </div>
          ...
          <div class="table-responsive text-nowrap" style="border-radius: 3px;">
            <table class="table table-striped">
              <thead class="table-dark">
                ...
                <th class="text-white">Account Creation Date</th>
                <th class="text-white">Account Update Date</th>
              ...
              <tbody class="table-border-bottom-0">
                ...
                <tr>
                  ...
                  <td>
                    <img src="@if(Storage::disk('public')->exists($user->image)) {{ asset('storage/'. $user->image) }} @else {{ asset('assets/img/'. $user->image) }} @endif" alt="Profile Photo - {{ $user->name }}" class="rounded avatar avatar-sm fotoProfile" data-url-img="@if(Storage::disk('public')->exists($user->image)) {{ asset('storage/'. $user->image) }} @else {{ asset('assets/img/'. $user->image) }} @endif" data-name-user="{{ $user->name }}">
                  </td>
                  <td class="text-capitalize">{{ $user->name }}</td>
                  <td>{{ '@' . $user->username }}</td>
                  <td>{{ substr($user->email , 0, 3) . str_repeat('*', strlen(substr($user->email, 0, strpos($user->email, '@'))) - 3) . substr($user->email, -10) }}</td>
                  <td>@if($user->gender == 'Male')<span class="badge bg-label-primary fw-bold">Male</span>@else<span class="badge fw-bold" style="color: #ff6384 !important; background-color: #ffe5eb !important;">Female</span>@endif</td>
                  <td>@if($user->tanggal_lahir) {{ \Carbon\Carbon::parse($user->tanggal_lahir)->locale('en')->isoFormat('D MMMM YYYY') }} @else User has not entered it yet! @endif</td>
                  <td>@if($user->alamat) {{ $user->alamat }} @else User has not entered it yet! @endif</td>
                  <td>{{ $user->created_at->locale('en')->isoFormat('D MMMM YYYY | H:mm') }}</td>
                  <td>{{ $user->updated_at->locale('en')->isoFormat('D MMMM YYYY | H:mm') }}</td>
                  <td class="text-center">
                    <button type="button" class="btn btn-icon btn-primary btn-sm buttonEditPengguna" data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="auto" title="Edit User" data-id="{{ encrypt($user->id) }}">
                      <span class="tf-icons bx bx-edit" style="font-size: 15px;"></span>
                    </button>
                    <button type="button" class="btn btn-icon btn-danger btn-sm buttonDeletePengguna" data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="auto" title="Delete User" data-id="{{ $user->id }}" data-name="{{ $user->name }}">
                ...
                @if($users->isEmpty())
                <tr>
                  <td colspan="100" class="text-center">Users Data Not Found dengan keyword pencarian: <b>"{{request('q')}}"</b></td>
                </tr>
                @endif
              </tbody>
            </table>
    ...
    <form action="/admin/pengguna" method="post" class="modalAdminAddPengguna" enctype="multipart/form-data">
      ...
      <div class="modal-content">
        <div class="modal-header d-flex justify-content-between">
          <h5 class="modal-title text-primary fw-bold">Add New User&nbsp;<i class='bx bxs-user fs-5' style="margin-bottom: 1px;"></i></h5>
          <button type="button" class="btn p-0 dropdown-toggle hide-arrow cancelModalAddUser" data-bs-dismiss="modal"><i class="bx bx-x-circle text-danger fs-4" data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="auto" title="Close"></i></button>
        </div>
        <div class="modal-body">
          <div class="row">
            <div class="col mb-3">
              <label for="nama_lengkap_user" class="form-label required-label">Full Name</label>
              <input type="text" id="nama_lengkap_user" name="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" placeholder="Enter user full name" autocomplete="off" required>
              @error('name')
          ...
          <div class="row">
            <div class="col mb-3">
              ...
              <input type="text" id="username_user" name="username" value="{{ old('username') }}" class="form-control @error('username') is-invalid @enderror" placeholder="Enter user username" autocomplete="off" required>
              @error('username')
              <div class="invalid-feedback" style="margin-bottom: -3px;">
                {{ $message }}
              </div>
          ...
          <div class="row">
            <div class="col mb-3">
              ...
              <input type="text" id="email_user" name="email" value="{{ old('email') }}" class="form-control @error('email') is-invalid @enderror" placeholder="Enter user email" autocomplete="off" required>
              @error('email')
              <div class="invalid-feedback" style="margin-bottom: -3px;">
                {{ $message }}
              </div>
          ...
          <div class="row">
            <div class="col mb-3">
              <label for="gender_user" class="form-label required-label">Gender</label>
          ...
          <div class="row">
            <div class="col mb-3">
              <label for="profil_user" class="form-label">Upload Photo</label>
          ...
          <div class="row">
            <div class="col">
              ...
              @endforeach
              @endif
              <div class="form-text @error('password') d-none @enderror"><i class='bx bx-error' style="font-size: 100%;"></i>&nbsp;Password must be at least 8 characters long and include uppercase and lowercase letters (A-Z, a-z), numbers (1-9), and special characters (@, #, %, etc.).</div>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-outline-danger cancelModalAddUser" data-bs-dismiss="modal"><i class='bx bx-share fs-6' style="margin-bottom: 3px;"></i>&nbsp;Cancel</button>
...
<div class="modal fade" id="formModalAdminEditPengguna" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <form action="/admin/pengguna/edit" method="post" class="modalAdminEditPengguna" enctype="multipart/form-data">
      ...
      <div class="modal-content">
        <div class="modal-header d-flex justify-content-between">
          <h5 class="modal-title text-primary fw-bold">Edit User&nbsp;<i class='bx bxs-user fs-5' style="margin-bottom: 1px;"></i></h5>
          <button type="button" class="btn p-0 dropdown-toggle hide-arrow cancelModalEditUser" data-bs-dismiss="modal"><i class="bx bx-x-circle text-danger fs-4" data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="auto" title="Close"></i></button>
        </div>
        <div class="modal-body">
          <div class="row">
            <div class="col mb-3">
              <label for="edit_nama_lengkap_user" class="form-label required-label">Full Name</label>
              <input type="text" id="edit_nama_lengkap_user" name="name" value="{{ old('name') }}" class="form-control @if(session('validationErrors')) @foreach (session('validationErrors')['name'] ?? [] as $errorMessage) is-invalid @endforeach @endif" placeholder="Enter user fullname" autocomplete="off" required>
          ...
          <div class="row">
            <div class="col mb-3">
              ...
              <input type="text" id="edit_username_user" name="username" value="{{ old('username') }}" class="form-control @if(session('validationErrors')) @foreach (session('validationErrors')['username'] ?? [] as $errorMessage) is-invalid @endforeach @endif" placeholder="Enter user username" autocomplete="off" required>
          ...
          <div class="row">
            <div class="col mb-3">
              ...
              <input type="text" id="edit_email_user" name="email" value="{{ old('email') }}" class="form-control @if(session('validationErrors')) @foreach (session('validationErrors')['email'] ?? [] as $errorMessage) is-invalid @endforeach @endif" placeholder="Enter user email" autocomplete="off" required>
          ...
          <div class="row">
            <div class="col mb-3">
              <label for="edit_gender_user" class="form-label required-label">Gender</label>
              <select class="form-select @if(session('validationErrors')) @foreach (session('validationErrors')['gender'] ?? [] as $errorMessage) is-invalid @endforeach @endif" name="gender" id="edit_gender_user" style="cursor: pointer;">
                <option value="" disabled selected>Select Gender</option>
                <option id="gender_laki-laki" @if(old('gender')=='Male' ) selected @endif value="Male">Male</option>
                <option id="gender_perempuan" @if(old('gender')=='Female' ) selected @endif value="Female">Female</option>
              </select>
          ...
          <div class="row">
            <div class="col mb-3">
              <label for="edit_profil_user" class="form-label">Upload Photo</label>
              ...
              @endforeach
              @endif
              <div class="form-text @if(session('validationErrors')) @foreach (session('validationErrors')['image'] ?? [] as $errorMessage) d-none @endforeach @endif">Maximum size 500 KB with a 1:1 ratio. Format: JPG, PNG, JPEG.</div>
            </div>
          </div>
          <div class="row">
            <div class="col">
              ...
              @endforeach
              @endif
              <div class="form-text @if(session('validationErrors')) @foreach (session('validationErrors')['password'] ?? [] as $errorMessage) d-none @endforeach @endif"><i class='bx bx-error' style="font-size: 100%;"></i>&nbsp;Password must be at least 8 characters long and include uppercase and lowercase letters (A-Z, a-z), numbers (1-9), and special characters (@, #, %, etc.).</div>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-outline-danger cancelModalEditUser" data-bs-dismiss="modal"><i class='bx bx-share fs-6' style="margin-bottom: 3px;"></i>&nbsp;Cancel</button>
